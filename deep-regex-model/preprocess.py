...
def get_data(args):
    ...
    char_indexer.add_w(
        [src_indexer.PAD, src_indexer.UNK, src_indexer.BOS, src_indexer.EOS]
    )

    def make_vocab(srcfile, targetfile, seqlength, max_word_l=0, chars=0):
        num_sents = 0
        for _, (src_orig, targ_orig) in enumerate(
            itertools.izip(open(srcfile, "r"), open(targetfile, "r"))
        ):
    ...
    def convert(
        ...
        for _, (src_orig, targ_orig) in enumerate(
            ...
            targ_orig = target_indexer.clean(targ_orig.decode("utf-8").strip())
            targ = (
                [target_indexer.BOS] + targ_orig.strip().split() + [target_indexer.EOS]
            )
            src = [src_indexer.BOS] + src_orig.strip().split() + [src_indexer.EOS]
            max_sent_l = max(len(targ), len(src), max_sent_l)
            if (
                len(targ) > newseqlength
                or len(src) > newseqlength
                or len(targ) < 3
                or len(src) < 3
            ):
                dropped += 1
            ...
            for word in targ:
                ...
                if chars == 1:
                    ...
                    char_idx = char_indexer.convert_sequence(
                        pad(char, max_word_l, char_indexer.PAD)
                    )
        ...
        for i in range(len(batch_idx) - 1):
            ...
            nonzeros.append(
                (target_output[batch_idx[i] - 1 : batch_idx[i + 1] - 1] != 1)
                .sum()
                .sum()
            )
    ...
    max_word_l, num_sents_train = make_vocab(
        args.srcfile, args.targetfile, args.seqlength, 0, args.chars
    )

    max_word_l, num_sents_valid = make_vocab(
        args.srcvalfile, args.targetvalfile, args.seqlength, max_word_l, args.chars
    )
    ...
    print(
        "Source vocab size: Original = {}, Pruned = {}".format(
            len(src_indexer.vocab), len(src_indexer.d)
        )
    )
    print(
        "Target vocab size: Original = {}, Pruned = {}".format(
            len(target_indexer.vocab), len(target_indexer.d)
        )
    )
...
def main(arguments):
    ...
    parser.add_argument(
        "--targetvocabsize",
        help="Size of target vocabulary, constructed "
        "by taking the top X most frequent words. "
        "Rest are replaced with special UNK tokens.",
        type=int,
        default=50000,
    ...
    parser.add_argument(
        "--srcfile",
        help="Path to source training data, "
    ...
    parser.add_argument(
        "--targetfile",
        help="Path to target training data, "
        "where each line represents a single "
        "source/target sequence.",
        required=True,
    )
    parser.add_argument(
        "--srcvalfile", help="Path to source validation data.", required=True
    )
    parser.add_argument(
        "--targetvalfile", help="Path to target validation data.", required=True
    )
    parser.add_argument(
        "--batchsize", help="Size of each minibatch.", type=int, default=32
    )
    parser.add_argument(
        "--seqlength",
    ...
    parser.add_argument(
        "--maxwordlength",
        help="For the character models, words are "
        "(if longer than maxwordlength) or zero-padded "
        "(if shorter) to maxwordlength",
        type=int,
        default=35,
    )
    parser.add_argument(
        "--chars",
        help="If 1, construct the character-level dataset as well. "
